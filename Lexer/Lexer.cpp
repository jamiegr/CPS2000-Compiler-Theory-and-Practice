...
Token Lexer::getNextToken() {
    if (line_cursor > total_lines || char_cursor >= program.length())
        return Token(TOK_EOF, "");//if last line is exceeded, return end of file token
    ...
    while (current_state != ERROR && char_cursor < program.length()) {
        ...
        current_lexeme.append(string(1,
                                     next));//using string(1, c) constructor to convert current character to a string and append it to the current lexeme
        if (final_states.find(current_state) != final_states.end())
            clearStack();//if current state is a final state clear stack
    ...
    unordered_map<int, TOK_TYPE>::const_iterator token = final_states.find(
            current_state);//searching for state in list of final states and returning resulting iterator
...
TOK_TYPE Lexer::classifyIdentifier(
        string identifier) {//checks if an identifier matches one of the token specific keywords and returns the TOK_TYPE accordingly
