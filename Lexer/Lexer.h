...
public:
    ...
    Token
    getNextToken();//get next token function to return next token in program for parser as specified in assignment description

private:
    function<bool(
            char)> classification_table[22] = {//array of functions to classify which character of the alphabet the next character falls under
